...
shinyServer(function(input, output) {
  ...
  # Render amortization input dynamically based on boxKI
  output$amortSlider <- renderUI({
    ...
    if (is.null(ki) || ki < 30) {
      ...
      numericInput("BoxAmort", "Amortering (%)", value = 2, min = 2, max = 15, step = 0.5)
    } else if (ki < 50) {
      # Medium LTV (50-70%): min 1% amortization
      numericInput("BoxAmort", "Amortering (%)", value = 1, min = 1, max = 15, step = 0.5)
    } else {
      # Low LTV (<50%): no minimum amortization
      numericInput("BoxAmort", "Amortering (%)", value = 0, min = 0, max = 15, step = 0.5)
    }
  })
